void Ext_Calib_Plunger(void){
  ...
  while(EXT_CAL > 0){
    ...
    while (pull == 0){  //Calibrate plunger MAX (Pull)
     ...
     NEW_Pos = map(analog_Z.getValue(),0,1024,0,255);                   // use the smoothed value 0-1024 (10bit) brought down to 0-255 scale (8bit) and apply calibration
    ...
    while (push == 0){  //Calibrate plunger MIN (Push)**************************************************************************************************
     ...
     NEW_Pos = map(analog_Z.getValue(),0,1024,0,255);    // use the smoothed value 0-1024 (10bit) brought down to 0-255 scale (8bit) and apply the offset
