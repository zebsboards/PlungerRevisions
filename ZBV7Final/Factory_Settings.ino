void FactoryReset (void)
{
  ...
  {
    1,                               // 0  - Gamepad/Keyboard Mode Switch  0 = Keyboard Mode / 1 = Gamepad Mode
    1,                               // 1  - Use Coindoor (0 = Not Used, 1 = Use Coin Door routine - Single Click)
    5,                               // 2  - button number assigned to Coin Door Input
    16,                              // 3  - Launch Ball Button Assignment (position in Keys array)
    28,                              // 4  - Launch Ball Keypress Assignment
    0,                               // 5  - (0 = No Hatswitch, 1 = Use Hatswitch)
    1,                               // 6  - nudging enabled = 1, OFF = 0
    4,                               // 7  - nudging sensitivity +-2g,4g,8g,16g
    7,                               // 8  - nudge sensor orientation (LScableDown = 0, CFcableDown = 1, RScableDown = 2, LScableFlat = 3, CFcableFrontFlat = 4, RScableFlat = 5, CBcableDown = 6, CFcableBackFlat = Default)
    100,                    				 // 9  - tilt debounce value
   	0,                 					     // 10 - n/a
    1,                               // 11 - nudge X axis amplification
    1,                               // 12 - nudge Y axis amplification
    8,                               // 13 - nudge X axis deadzone value
    8,                               // 14 - nudge Y axis deadzone value
    0,                               // 15 - (1 = Enable Tilt routines, 0 = Disable tilt routines)
    17,                              // 16 - button number assigned to Tilt input
    147,                             // 17 - XTilt value received
    147,                             // 18 - YTilt value received
    0,                               // 19 - n/a
    0,                               // 20 - n/a
    0,                               // 21 - n/a
    0,                               // 22 - n/a
  ...
  Serial.println("A,B");                                                         // send ack and reload trigger to utility
  delay(100);
  Serial.println("M,Factory Settings Restored Successfully");
}